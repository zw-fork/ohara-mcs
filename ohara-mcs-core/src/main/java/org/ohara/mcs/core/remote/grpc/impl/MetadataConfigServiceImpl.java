package org.ohara.mcs.core.remote.grpc.impl;
...
import org.ohara.mcs.core.remote.raft.client.RaftClientFactory;
import org.ohara.mcs.core.remote.raft.client.RaftClientWorker;
...
public class MetadataConfigServiceImpl extends MetadataServiceGrpc.MetadataServiceImplBase implements GrpcService {
    ...
    public class MetadataChangeListener implements Listener<MetadataChangeEvent> {
        ...
        public void onSubscribe(MetadataChangeEvent event) {
            ...
            Log.print("MetadataChangeListener->收到监听消息, type=%s metadata=%s", event.type(), event.metadata());
    ...
    private static class SubscribeObserver {
        ...
        SubscribeObserver(String namespace, StreamObserver<MetadataSubscribeResponse> responseObserver) {
     ...
     */
    @Override
    public void subscribe(MetadataSubscribeRequest request, StreamObserver<MetadataSubscribeResponse> responseObserver) {
        SubscribeObserver subscribeObserver = new SubscribeObserver(request.getNamespace(), responseObserver);
        subscribers.add(subscribeObserver);

        // Automatically cleans up disconnected connections
        Context.current().addListener(
                context -> subscribers.remove(subscribeObserver), MoreExecutors.directExecutor()
        );
    }
    ...
    public void get(MetadataReadRequest request, StreamObserver<Response> responseObserver) {
        try {
            ...
            String key = storage.key(request.getNamespace(), request.getGroup(), request.getTag(), request.getDataId());
        ...
        while (it.hasNext()) {
            ...
            try {
                if (isMatch(metadata, subscribe) && !eventType.isGet()) {
                    MetadataSubscribeResponse response = buildSubscribeResponse(metadata, eventType);
                    // TODO 采集广播日志
    ...
    private MetadataSubscribeResponse buildSubscribeResponse(Metadata metadata, EventType eventType){
