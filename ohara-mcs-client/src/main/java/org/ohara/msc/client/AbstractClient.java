...
public abstract class AbstractClient<OPTION extends RequestOption> implements LeftCycle {
    ...
    public void init() {
        ...
        synchronized (this.initLock) {
            ...
            try {
                ...
                String errLog = MessageFormat.format("[Client-Main-Start] Initialize LinkClient fail, {0}", ex.getMessage());
                Log.error(errLog, ex);
    ...
    public void destroy(Integer timeout, TimeUnit unit) {
        ...
        synchronized (this.initLock) {
            ...
            try {
                ...
                String errLog = MessageFormat.format("[Client-Main-Destroy] LinkClient destroy fail, {0}", ex.getMessage());
                Log.error(errLog, ex);
    ...
    private void buildFilterChain() throws InitializationException {
        try {
            ...
            throw new InitializationException(MessageFormat.format("[Client-Main-Start] Create FilterChain fail, {0}", e.getMessage()), e);
        }
    }
